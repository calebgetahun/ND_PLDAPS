...
p.trial.reward.IncrementDur   = [0.75, 0.75, 0.75, 0.75, 0.75, 0.75, 0.75]; % increase number of pulses with this trial number
...
% Main trial timings
p.trial.task.stimLatency      = ND_GetITI(0.2, .2); % Time from fixation onset to stim appearing

p.trial.task.saccadeTimeout   = 1.0;   % Time allowed to make the saccade to the stim before error
...
p.trial.task.minTargetFixTime = .2;  % Must fixate on target for at least this time before it counts
...
p.trial.task.Timing.ITI       = ND_GetITI(1,  1,  1, 1, 1, 1);
...
p.trial.stim.GRATING.fixWin = 2.5;  %*p.trial.stim.GRATING.radius;
