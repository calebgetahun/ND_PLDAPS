...
% old code from wolf that wasn't robust to differing numbers of specified parameters (e.g. three orientations, one size)
%     % Gratings
%     p.trial.stim.gratings = {};
%       p.trial.stim.count    =  0;
%     
            ...
            if p.trial.stim.fix.fixating
                ...
                if(p.trial.CurTime < p.trial.stim.fix.EV.FixStart + p.trial.task.Timing.jackpotTime)
                    ...
                    if p.trial.stim.count > length(p.trial.stim.iStim)
                        ...
                        % When RFmeth is switched to fine, count is reset at 0, display no stims and wait for jackpot
                if p.trial.stim.count == 0
                    stim(p,0);
                    return;
                end
                    ...
                    if(p.trial.reward.GiveSeries==1)
                        cstep = find(p.trial.reward.Step <= p.trial.reward.iReward, 1, 'last');
...
%                         StimLstPtr = fopen(p.trial.stimbl.file, 'w');
